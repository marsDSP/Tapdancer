...
def generate_svg(data, filename="cos_error_visualization.svg"):
    ...
    with open(filename, "w") as f:
        ...
        # Title
        f.write(f'<text x="{width//2}" y="35" text-anchor="middle" font-family="sans-serif" font-size="24" font-weight="bold">SIMD Pade Cosine Approximant Validation</text>\n')

        # Top Panel
        f.write(f'<text x="{margin}" y="{margin-15}" font-family="sans-serif" font-size="16" font-weight="bold">cos(x) Comparison</text>\n')
        ...
        f.write(f'<text x="{margin+20}" y="{margin+30}" fill="#3498db" font-family="sans-serif" font-size="12">Solid: std::cos</text>\n')
...
if __name__ == "__main__":
    ...
    csv_file = os.path.join(script_dir, "logs", "simd_cos_results.csv")
    output_file = os.path.join(script_dir, "logs", "cos_error_visualization.svg")
